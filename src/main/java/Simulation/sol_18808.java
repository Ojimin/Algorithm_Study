...
// 다시 - 직사각형 회전!! & 새롭게 배치 시, 기존 스티커에 +row, column 값 더하기 & 시간복잡도 -
// 최적화 -
public class sol_18808 {
    static int N, M, K;
    static int[][] map;
...
//        boolean visited[][] = new boolean[sticker.length][sticker[0].length];
...
//                int nx = now[0] + dx[i];
//                int ny = now[1] + dy[i];
//                if (nx<0 || nx >= sticker.length || ny<0 || ny>= sticker[0].length || visited[nx][ny]) continue;
//                if (nx+row<0 || nx+row >= N || ny+col<0 || ny+col>=M) return false;
//                if (map[nx+row][ny+col] == 1) return false;
//                visited[nx][ny] = true;
//                if (sticker[nx][ny] == 1) q.offer(new Integer[]{nx, ny});
//            }
//        }
