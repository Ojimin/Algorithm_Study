...
// 반례 : 중간에 2,3방향으로 가는 케이스 생각 못했음 + 이걸 해결하기 위해서는 두번쨰 depth에서 한번더 탐색할 수 있게 함
public class sol_14500 {
    static int N, M, map[][];
...
//        int threeWay = 0;
        for (int i=0; i<4; i++) {
            ...
            if (depth == 2) {
                visited[nx][ny] = true;
                dfs(x, y, depth+1, cnt+map[nx][ny]); // 이렇게 호출하면 굳이 해당 좌표 dfs안해도 탐색 ㅇㅋ함
                visited[nx][ny] = false;
            }
            visited[nx][ny] = true;
